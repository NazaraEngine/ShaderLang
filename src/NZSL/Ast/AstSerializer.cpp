...
namespace nzsl::Ast
{
	namespace
	{
		...
		constexpr std::uint32_t s_shaderAstCurrentVersion = 3;
...
#include <NZSL/Ast/NodeList.hpp>
	...
	void SerializerBase::Serialize(ConstantArrayValueExpression& node)
	{
		...
		static_assert(std::variant_size_v<decltype(node.values)> == 19);
		switch (typeIndex)
		{
			case 0:  break;
	...
	void SerializerBase::Serialize(ConstantValueExpression& node)
	{
		...
		static_assert(std::variant_size_v<decltype(node.value)> == 19);
		switch (typeIndex)
		{
			case 0:  break;
			case 1:  SerializeValue(bool()); break;
