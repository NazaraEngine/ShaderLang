...
NZSL_SHADERLANG_COMPILER_ERROR(AssignTemporary, "constant and temporary values cannot be assigned")
...
NZSL_SHADERLANG_COMPILER_ERROR(ExpectedConstantType, "const and option types can only be scalars/vectors (or arrays of scalars/vectors), got {}", std::string)
...
NZSL_SHADERLANG_COMPILER_ERROR(FieldUnexpectedType, "unexpected type: only structs can be field indexed (got {})", std::string)
...
NZSL_SHADERLANG_COMPILER_ERROR(FunctionCallSemanticRequiresVariable, "this semantic requires a variable")
...
NZSL_SHADERLANG_AST_ERROR(ExpectedIndex, "a {} index is missing", std::string_view)
...
NZSL_SHADERLANG_AST_ERROR(UnresolvedExpressionType, "unresolved expression type in a non-partial compilation")
