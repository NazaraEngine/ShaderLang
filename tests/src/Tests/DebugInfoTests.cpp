#include <Tests/ShaderUtils.hpp>
#include <NZSL/FilesystemModuleResolver.hpp>
#include <NZSL/ShaderBuilder.hpp>
#include <NZSL/Parser.hpp>
#include <catch2/catch_test_macros.hpp>
#include <cctype>

TEST_CASE("debug info", "[Shader]")
{
	SECTION("Generate debug info for a simple file")
	{
		auto directoryModuleResolver = std::make_shared<nzsl::FilesystemModuleResolver>();
		directoryModuleResolver->RegisterDirectory("../resources/modules");

		nzsl::Ast::ModulePtr shaderModule = nzsl::ParseFromFile("../resources/Shader.nzsl");

		nzsl::Ast::ResolveTransformer::Options resolverOptions;
		resolverOptions.moduleResolver = directoryModuleResolver;

		ResolveOptions resolveOptions;
		resolveOptions.identifierResolverOptions = &resolverOptions;

		ResolveModule(*shaderModule, resolveOptions);

		WHEN("Generating with no debug info")
		{
			nzsl::BackendParameters options;
			options.debugLevel = nzsl::DebugLevel::None;

			ExpectGLSL(*shaderModule, R"(
#version 300 es

precision highp int;
#if GL_FRAGMENT_PRECISION_HIGH
precision highp float;
precision highp sampler2D;
#else
precision mediump float;
precision mediump sampler2D;
#endif

uniform sampler2D tex1_Color;

vec4 GenerateColor_Color()
{
	return texture(tex1_Color, vec2(0.0, 0.0));
}

vec4 GetColor_Color()
{
	return GenerateColor_Color();
}

struct Data_DataStruct
{
	vec4 color;
};

vec4 GetColorFromData_OutputStruct(Data_DataStruct data)
{
	return data.color * vec4(0.5, 0.5, 0.5, 1.0);
}

struct Output_OutputStruct
{
	vec4 color;
};

struct PushConstants
{
	vec4 color;
};

layout(std140) uniform _nzslPushConstant
{
	vec4 color;
} ExternalResources_constants;

layout(location = 0) out vec4 _nzslOutcolor;

void main()
{
	Data_DataStruct data;
	data.color = GetColor_Color();
	Output_OutputStruct output_;
	output_.color = (GetColorFromData_OutputStruct(data)) * ExternalResources_constants.color;

	_nzslOutcolor = output_.color;
	return;
})", options);

			ExpectSPIRV(*shaderModule, R"(
      OpCapability Capability(Shader)
      OpMemoryModel AddressingModel(Logical) MemoryModel(GLSL450)
      OpEntryPoint ExecutionModel(Fragment) %31 "main" %25
      OpExecutionMode %31 ExecutionMode(OriginUpperLeft)
      OpDecorate %5 Decoration(Binding) 0
      OpDecorate %5 Decoration(DescriptorSet) 0
      OpDecorate %25 Decoration(Location) 0
      OpMemberDecorate %11 0 Decoration(Offset) 0
      OpDecorate %19 Decoration(Block)
      OpMemberDecorate %19 0 Decoration(Offset) 0
      OpMemberDecorate %26 0 Decoration(Offset) 0
 %1 = OpTypeFloat 32
 %2 = OpTypeImage %1 Dim(Dim2D) 0 0 0 1 ImageFormat(Unknown)
 %3 = OpTypeSampledImage %2
 %4 = OpTypePointer StorageClass(UniformConstant) %3
 %6 = OpTypeVector %1 4
 %7 = OpTypeFunction %6
 %8 = OpTypeVector %1 2
 %9 = OpConstant %1 f32(0)
%10 = OpConstantComposite %8 %9 %9
%11 = OpTypeStruct %6
%12 = OpTypePointer StorageClass(Function) %11
%13 = OpTypeFunction %6 %12
%14 = OpTypeInt 32 1
%15 = OpConstant %14 i32(0)
%16 = OpConstant %1 f32(0.5)
%17 = OpConstant %1 f32(1)
%18 = OpConstantComposite %6 %16 %16 %16 %17
%19 = OpTypeStruct %6
%20 = OpTypePointer StorageClass(PushConstant) %19
%22 = OpTypeVoid
%23 = OpTypeFunction %22
%24 = OpTypePointer StorageClass(Output) %6
%26 = OpTypeStruct %6
%27 = OpTypePointer StorageClass(Function) %26
%39 = OpTypePointer StorageClass(Function) %6
%51 = OpTypePointer StorageClass(PushConstant) %6
 %5 = OpVariable %4 StorageClass(UniformConstant)
%21 = OpVariable %20 StorageClass(PushConstant)
%25 = OpVariable %24 StorageClass(Output)
%28 = OpFunction %6 FunctionControl(0) %7
%32 = OpLabel
%33 = OpLoad %3 %5
%34 = OpImageSampleImplicitLod %6 %33 %10
      OpReturnValue %34
      OpFunctionEnd
%29 = OpFunction %6 FunctionControl(0) %7
%35 = OpLabel
%36 = OpFunctionCall %6 %28
      OpReturnValue %36
      OpFunctionEnd
%30 = OpFunction %6 FunctionControl(0) %13
%37 = OpFunctionParameter %12
%38 = OpLabel
%40 = OpAccessChain %39 %37 %15
%41 = OpLoad %6 %40
%42 = OpFMul %6 %41 %18
      OpReturnValue %42
      OpFunctionEnd
%31 = OpFunction %22 FunctionControl(0) %23
%43 = OpLabel
%44 = OpVariable %12 StorageClass(Function)
%45 = OpVariable %27 StorageClass(Function)
%46 = OpVariable %12 StorageClass(Function)
%47 = OpFunctionCall %6 %29
%48 = OpAccessChain %39 %44 %15
      OpStore %48 %47
%49 = OpLoad %11 %44
      OpStore %46 %49
%50 = OpFunctionCall %6 %30 %46
%52 = OpAccessChain %51 %21 %15
%53 = OpLoad %6 %52
%54 = OpFMul %6 %50 %53
%55 = OpAccessChain %39 %45 %15
      OpStore %55 %54
%56 = OpLoad %26 %45
%57 = OpCompositeExtract %6 %56 0
      OpStore %25 %57
      OpReturn
      OpFunctionEnd)", options, {}, true);
		}

		WHEN("Generating with minimal info")
		{
			nzsl::BackendParameters options;
			options.debugLevel = nzsl::DebugLevel::Minimal;

			ExpectGLSL(*shaderModule, R"(
#version 300 es

// fragment shader - this file was generated by NZSL compiler (Nazara Shading Language)

precision highp int;
#if GL_FRAGMENT_PRECISION_HIGH
precision highp float;
precision highp sampler2D;
#else
precision mediump float;
precision mediump sampler2D;
#endif

// header end

// Module Color
// Author: SirLynix
// Description: Test color module
// License: MIT

uniform sampler2D tex1_Color;

vec4 GenerateColor_Color()
{
	return texture(tex1_Color, vec2(0.0, 0.0));
}

vec4 GetColor_Color()
{
	return GenerateColor_Color();
}

// Module DataStruct

struct Data_DataStruct
{
	vec4 color;
};

// Module OutputStruct

vec4 GetColorFromData_OutputStruct(Data_DataStruct data)
{
	return data.color * vec4(0.5, 0.5, 0.5, 1.0);
}

struct Output_OutputStruct
{
	vec4 color;
};

// Main module
// Author: SirLynix
// Description: Test module
// License: MIT

struct PushConstants
{
	vec4 color;
};

layout(std140) uniform _nzslPushConstant
{
	vec4 color;
} ExternalResources_constants;

/*************** Outputs ***************/
layout(location = 0) out vec4 _nzslOutcolor;

void main()
{
	Data_DataStruct data;
	data.color = GetColor_Color();
	Output_OutputStruct output_;
	output_.color = (GetColorFromData_OutputStruct(data)) * ExternalResources_constants.color;

	_nzslOutcolor = output_.color;
	return;
})", options);

			ExpectSPIRV(*shaderModule, R"(
      OpCapability Capability(Shader)
      OpMemoryModel AddressingModel(Logical) MemoryModel(GLSL450)
      OpEntryPoint ExecutionModel(Fragment) %31 "main" %25
      OpExecutionMode %31 ExecutionMode(OriginUpperLeft)
      OpSource SourceLanguage(NZSL) 4198400
      OpSourceExtension "Version: 1.1"
      OpName %11 "Data"
      OpMemberName %11 0 "color"
      OpName %19 "PushConstants"
      OpMemberName %19 0 "color"
      OpName %26 "Output"
      OpMemberName %26 0 "color"
      OpName %5 "tex1"
      OpName %21 "ExternalResources_constants"
      OpName %25 "color"
      OpName %28 "GenerateColor"
      OpName %29 "GetColor"
      OpName %30 "GetColorFromData"
      OpName %31 "main"
      OpDecorate %5 Decoration(Binding) 0
      OpDecorate %5 Decoration(DescriptorSet) 0
      OpDecorate %25 Decoration(Location) 0
      OpMemberDecorate %11 0 Decoration(Offset) 0
      OpDecorate %19 Decoration(Block)
      OpMemberDecorate %19 0 Decoration(Offset) 0
      OpMemberDecorate %26 0 Decoration(Offset) 0
 %1 = OpTypeFloat 32
 %2 = OpTypeImage %1 Dim(Dim2D) 0 0 0 1 ImageFormat(Unknown)
 %3 = OpTypeSampledImage %2
 %4 = OpTypePointer StorageClass(UniformConstant) %3
 %6 = OpTypeVector %1 4
 %7 = OpTypeFunction %6
 %8 = OpTypeVector %1 2
 %9 = OpConstant %1 f32(0)
%10 = OpConstantComposite %8 %9 %9
%11 = OpTypeStruct %6
%12 = OpTypePointer StorageClass(Function) %11
%13 = OpTypeFunction %6 %12
%14 = OpTypeInt 32 1
%15 = OpConstant %14 i32(0)
%16 = OpConstant %1 f32(0.5)
%17 = OpConstant %1 f32(1)
%18 = OpConstantComposite %6 %16 %16 %16 %17
%19 = OpTypeStruct %6
%20 = OpTypePointer StorageClass(PushConstant) %19
%22 = OpTypeVoid
%23 = OpTypeFunction %22
%24 = OpTypePointer StorageClass(Output) %6
%26 = OpTypeStruct %6
%27 = OpTypePointer StorageClass(Function) %26
%39 = OpTypePointer StorageClass(Function) %6
%51 = OpTypePointer StorageClass(PushConstant) %6
 %5 = OpVariable %4 StorageClass(UniformConstant)
%21 = OpVariable %20 StorageClass(PushConstant)
%25 = OpVariable %24 StorageClass(Output)
%28 = OpFunction %6 FunctionControl(0) %7
%32 = OpLabel
%33 = OpLoad %3 %5
%34 = OpImageSampleImplicitLod %6 %33 %10
      OpReturnValue %34
      OpFunctionEnd
%29 = OpFunction %6 FunctionControl(0) %7
%35 = OpLabel
%36 = OpFunctionCall %6 %28
      OpReturnValue %36
      OpFunctionEnd
%30 = OpFunction %6 FunctionControl(0) %13
%37 = OpFunctionParameter %12
%38 = OpLabel
%40 = OpAccessChain %39 %37 %15
%41 = OpLoad %6 %40
%42 = OpFMul %6 %41 %18
      OpReturnValue %42
      OpFunctionEnd
%31 = OpFunction %22 FunctionControl(0) %23
%43 = OpLabel
%44 = OpVariable %12 StorageClass(Function)
%45 = OpVariable %27 StorageClass(Function)
%46 = OpVariable %12 StorageClass(Function)
%47 = OpFunctionCall %6 %29
%48 = OpAccessChain %39 %44 %15
      OpStore %48 %47
%49 = OpLoad %11 %44
      OpStore %46 %49
%50 = OpFunctionCall %6 %30 %46
%52 = OpAccessChain %51 %21 %15
%53 = OpLoad %6 %52
%54 = OpFMul %6 %50 %53
%55 = OpAccessChain %39 %45 %15
      OpStore %55 %54
%56 = OpLoad %26 %45
%57 = OpCompositeExtract %6 %56 0
      OpStore %25 %57
      OpReturn
      OpFunctionEnd)", options, {}, true);
		}

		WHEN("Generating with regular debug info")
		{
			nzsl::BackendParameters options;
			options.debugLevel = nzsl::DebugLevel::Regular;

			ExpectGLSL(*shaderModule, R"(
#version 300 es

// fragment shader - this file was generated by NZSL compiler (Nazara Shading Language)

precision highp int;
#if GL_FRAGMENT_PRECISION_HIGH
precision highp float;
precision highp sampler2D;
#else
precision mediump float;
precision mediump sampler2D;
#endif

// header end

// Module Color
// NZSL version: 1.1
// from ../resources/modules/Color.nzsl
// Author: SirLynix
// Description: Test color module
// License: MIT

// @../resources/modules/Color.nzsl:8:1
uniform sampler2D tex1_Color;

// @../resources/modules/Color.nzsl:13:1
vec4 GenerateColor_Color()
{
	return texture(tex1_Color, vec2(0.0, 0.0));
}

// @../resources/modules/Color.nzsl:32:1
vec4 GetColor_Color()
{
	return GenerateColor_Color();
}

// Module DataStruct
// NZSL version: 1.0

// @../resources/modules/Data/DataStruct.nzsl:5:1
struct Data_DataStruct
{
	vec4 color;
};

// Module OutputStruct
// NZSL version: 1.1
// from ../resources/modules/Data/OutputStruct.nzsl

// @../resources/modules/Data/OutputStruct.nzsl:9:1
vec4 GetColorFromData_OutputStruct(Data_DataStruct data)
{
	return data.color * vec4(0.5, 0.5, 0.5, 1.0);
}

// @../resources/modules/Data/OutputStruct.nzsl:15:1
struct Output_OutputStruct
{
	vec4 color;
};

// Main module
// NZSL version: 1.1
// from ../resources/Shader.nzsl
// Author: SirLynix
// Description: Test module
// License: MIT

// @../resources/Shader.nzsl:12:1
struct PushConstants
{
	vec4 color;
};

// @../resources/Shader.nzsl:17:1
layout(std140) uniform _nzslPushConstant
{
	vec4 color;
} ExternalResources_constants;

/*************** Outputs ***************/
layout(location = 0) out vec4 _nzslOutcolor;

// @../resources/Shader.nzsl:23:1
void main()
{
	Data_DataStruct data;
	data.color = GetColor_Color();
	Output_OutputStruct output_;
	output_.color = (GetColorFromData_OutputStruct(data)) * ExternalResources_constants.color;

	_nzslOutcolor = output_.color;
	return;
})", options);

			ExpectSPIRV(*shaderModule, R"(
      OpCapability Capability(Shader)
      OpMemoryModel AddressingModel(Logical) MemoryModel(GLSL450)
      OpEntryPoint ExecutionModel(Fragment) %31 "main" %25
      OpExecutionMode %31 ExecutionMode(OriginUpperLeft)
%32 = OpString "../resources/Shader.nzsl"
%33 = OpString "../resources/modules/Color.nzsl"
%34 = OpString "../resources/modules/Data/OutputStruct.nzsl"
      OpSource SourceLanguage(NZSL) 4198400 %32
      OpSourceExtension "Version: 1.1"
      OpSourceExtension "ModuleName: Shader"
      OpSourceExtension "Author: SirLynix"
      OpSourceExtension "Description: Test module"
      OpSourceExtension "License: MIT"
      OpSource SourceLanguage(NZSL) 4198400 %33
      OpSourceExtension "Version: 1.1"
      OpSourceExtension "ModuleName: Color"
      OpSourceExtension "Author: SirLynix"
      OpSourceExtension "Description: Test color module"
      OpSourceExtension "License: MIT"
      OpSource SourceLanguage(NZSL) 4194304
      OpSourceExtension "Version: 1.0"
      OpSourceExtension "ModuleName: DataStruct"
      OpSource SourceLanguage(NZSL) 4198400 %34
      OpSourceExtension "Version: 1.1"
      OpSourceExtension "ModuleName: OutputStruct"
      OpName %11 "Data"
      OpMemberName %11 0 "color"
      OpName %19 "PushConstants"
      OpMemberName %19 0 "color"
      OpName %26 "Output"
      OpMemberName %26 0 "color"
      OpName %5 "tex1"
      OpName %21 "ExternalResources_constants"
      OpName %25 "color"
      OpName %28 "GenerateColor"
      OpName %29 "GetColor"
      OpName %30 "GetColorFromData"
      OpName %31 "main"
      OpDecorate %5 Decoration(Binding) 0
      OpDecorate %5 Decoration(DescriptorSet) 0
      OpDecorate %25 Decoration(Location) 0
      OpMemberDecorate %11 0 Decoration(Offset) 0
      OpDecorate %19 Decoration(Block)
      OpMemberDecorate %19 0 Decoration(Offset) 0
      OpMemberDecorate %26 0 Decoration(Offset) 0
 %1 = OpTypeFloat 32
 %2 = OpTypeImage %1 Dim(Dim2D) 0 0 0 1 ImageFormat(Unknown)
 %3 = OpTypeSampledImage %2
 %4 = OpTypePointer StorageClass(UniformConstant) %3
 %6 = OpTypeVector %1 4
 %7 = OpTypeFunction %6
 %8 = OpTypeVector %1 2
 %9 = OpConstant %1 f32(0)
%10 = OpConstantComposite %8 %9 %9
%11 = OpTypeStruct %6
%12 = OpTypePointer StorageClass(Function) %11
%13 = OpTypeFunction %6 %12
%14 = OpTypeInt 32 1
%15 = OpConstant %14 i32(0)
%16 = OpConstant %1 f32(0.5)
%17 = OpConstant %1 f32(1)
%18 = OpConstantComposite %6 %16 %16 %16 %17
%19 = OpTypeStruct %6
%20 = OpTypePointer StorageClass(PushConstant) %19
%22 = OpTypeVoid
%23 = OpTypeFunction %22
%24 = OpTypePointer StorageClass(Output) %6
%26 = OpTypeStruct %6
%27 = OpTypePointer StorageClass(Function) %26
%42 = OpTypePointer StorageClass(Function) %6
%54 = OpTypePointer StorageClass(PushConstant) %6
 %5 = OpVariable %4 StorageClass(UniformConstant)
%21 = OpVariable %20 StorageClass(PushConstant)
%25 = OpVariable %24 StorageClass(Output)
      OpLine %33 13 1
%28 = OpFunction %6 FunctionControl(0) %7
%35 = OpLabel
      OpNoLine
      OpLine %33 15 5
      OpLine %33 15 12
%36 = OpLoad %3 %5
%37 = OpImageSampleImplicitLod %6 %36 %10
      OpReturnValue %37
      OpFunctionEnd
      OpLine %33 32 1
%29 = OpFunction %6 FunctionControl(0) %7
%38 = OpLabel
      OpNoLine
      OpLine %33 34 5
      OpLine %33 34 12
%39 = OpFunctionCall %6 %28
      OpReturnValue %39
      OpFunctionEnd
      OpLine %34 9 1
%30 = OpFunction %6 FunctionControl(0) %13
      OpLine %34 9 21
%40 = OpFunctionParameter %12
%41 = OpLabel
      OpNoLine
      OpLine %34 11 5
      OpLine %34 11 12
%43 = OpAccessChain %42 %40 %15
%44 = OpLoad %6 %43
%45 = OpFMul %6 %44 %18
      OpReturnValue %45
      OpFunctionEnd
      OpLine %32 23 1
%31 = OpFunction %22 FunctionControl(0) %23
%46 = OpLabel
      OpNoLine
      OpLine %32 25 5
%47 = OpVariable %12 StorageClass(Function)
      OpLine %32 28 5
%48 = OpVariable %27 StorageClass(Function)
      OpLine %32 29 37
%49 = OpVariable %12 StorageClass(Function)
      OpLine %32 26 18
%50 = OpFunctionCall %6 %29
      OpLine %32 26 5
%51 = OpAccessChain %42 %47 %15
      OpStore %51 %50
      OpLine %32 29 37
%52 = OpLoad %11 %47
      OpStore %49 %52
      OpLine %32 29 20
%53 = OpFunctionCall %6 %30 %49
      OpLine %32 29 45
%55 = OpAccessChain %54 %21 %15
%56 = OpLoad %6 %55
      OpLine %32 29 20
%57 = OpFMul %6 %53 %56
      OpLine %32 29 5
%58 = OpAccessChain %42 %48 %15
      OpStore %58 %57
      OpLine %32 31 12
%59 = OpLoad %26 %48
%60 = OpCompositeExtract %6 %59 0
      OpStore %25 %60
      OpLine %32 31 5
      OpReturn
      OpFunctionEnd)", options, {}, true);
		}
		
		WHEN("Generating with full debug info")
		{
			nzsl::BackendParameters options;
			options.debugLevel = nzsl::DebugLevel::Full;

			ExpectGLSL(*shaderModule, R"(
#version 300 es

// fragment shader - this file was generated by NZSL compiler (Nazara Shading Language)

precision highp int;
#if GL_FRAGMENT_PRECISION_HIGH
precision highp float;
precision highp sampler2D;
#else
precision mediump float;
precision mediump sampler2D;
#endif

// header end

// Module Color
// NZSL version: 1.1
// from ../resources/modules/Color.nzsl
#if 0 // Module source code

[nzsl_version("1.1")]
[author("SirLynix")]
[desc("Test color module")]
[license("MIT")]
module Color;

[set(0)]
external
{
    [binding(0)] tex1: sampler2D[f32]
}

fn GenerateColor() -> vec4[f32]
{
    return tex1.Sample(0.0.xx);
}

alias GenColor = GenerateColor;

[set(1)]
external
{
    [binding(0)] tex2: sampler2D[f32]
}

fn GenerateAnotherColor() -> vec4[f32]
{
    return tex2.Sample(0.5.xx);
}

[export]
fn GetColor() -> vec4[f32]
{
    return GenColor();
}

[export]
fn GetAnotherColor() -> vec4[f32]
{
    return GenerateAnotherColor();
}

#endif // Module source code

// Author: SirLynix
// Description: Test color module
// License: MIT

// @../resources/modules/Color.nzsl:8:1
uniform sampler2D tex1_Color;

// @../resources/modules/Color.nzsl:13:1
vec4 GenerateColor_Color()
{
	return texture(tex1_Color, vec2(0.0, 0.0));
}

// @../resources/modules/Color.nzsl:32:1
vec4 GetColor_Color()
{
	return GenerateColor_Color();
}

// Module DataStruct
// NZSL version: 1.0

// @../resources/modules/Data/DataStruct.nzsl:5:1
struct Data_DataStruct
{
	vec4 color;
};

// Module OutputStruct
// NZSL version: 1.1
// from ../resources/modules/Data/OutputStruct.nzsl
#if 0 // Module source code

[nzsl_version("1.1")]
module OutputStruct;

import * from DataStruct;

option ColorMultiplier: vec4[f32] = vec4[f32](0.5, 0.5, 0.5, 1.0);

[export]
fn GetColorFromData(data: Data) -> vec4[f32]
{
    return data.color * ColorMultiplier;
}

[export]
struct Output
{
    [location(0)] color: vec4[f32]
}

#endif // Module source code

// @../resources/modules/Data/OutputStruct.nzsl:9:1
vec4 GetColorFromData_OutputStruct(Data_DataStruct data)
{
	return data.color * vec4(0.5, 0.5, 0.5, 1.0);
}

// @../resources/modules/Data/OutputStruct.nzsl:15:1
struct Output_OutputStruct
{
	vec4 color;
};

// Main module
// NZSL version: 1.1
// from ../resources/Shader.nzsl
#if 0 // Module source code

[nzsl_version("1.1")]
[author("SirLynix")]
[desc("Test module")]
[license("MIT")]
module Shader;

import GetColor as Color from Color;
import * from DataStruct;
import * from OutputStruct;

[layout(std140)]
struct PushConstants
{
    color: vec4[f32]
}

external ExternalResources
{
    constants: push_constant[PushConstants]
}

[entry(frag)]
fn main() -> Output
{
    let data: Data;
    data.color = Color();

    let output: Output;
    output.color = GetColorFromData(data) * ExternalResources.constants.color;

    return output;
}

#endif // Module source code

// Author: SirLynix
// Description: Test module
// License: MIT

// @../resources/Shader.nzsl:12:1
struct PushConstants
{
	vec4 color;
};

// @../resources/Shader.nzsl:17:1
layout(std140) uniform _nzslPushConstant
{
	vec4 color;
} ExternalResources_constants;

/*************** Outputs ***************/
layout(location = 0) out vec4 _nzslOutcolor;

// @../resources/Shader.nzsl:23:1
void main()
{
	Data_DataStruct data;
	data.color = GetColor_Color();
	Output_OutputStruct output_;
	output_.color = (GetColorFromData_OutputStruct(data)) * ExternalResources_constants.color;

	_nzslOutcolor = output_.color;
	return;
})", options);

			ExpectSPIRV(*shaderModule, R"(
      OpCapability Capability(Shader)
      OpMemoryModel AddressingModel(Logical) MemoryModel(GLSL450)
      OpEntryPoint ExecutionModel(Fragment) %31 "main" %25
      OpExecutionMode %31 ExecutionMode(OriginUpperLeft)
%32 = OpString "../resources/Shader.nzsl"
%33 = OpString "../resources/modules/Color.nzsl"
%34 = OpString "../resources/modules/Data/OutputStruct.nzsl"
      OpSource SourceLanguage(NZSL) 4198400 %32 "[nzsl_version("1.1")]
[author("SirLynix")]
[desc("Test module")]
[license("MIT")]
module Shader;

import GetColor as Color from Color;
import * from DataStruct;
import * from OutputStruct;

[layout(std140)]
struct PushConstants
{
    color: vec4[f32]
}

external ExternalResources
{
    constants: push_constant[PushConstants]
}

[entry(frag)]
fn main() -> Output
{
    let data: Data;
    data.color = Color();

    let output: Output;
    output.color = GetColorFromData(data) * ExternalResources.constants.color;

    return output;
}
"
      OpSourceExtension "Version: 1.1"
      OpSourceExtension "ModuleName: Shader"
      OpSourceExtension "Author: SirLynix"
      OpSourceExtension "Description: Test module"
      OpSourceExtension "License: MIT"
      OpSource SourceLanguage(NZSL) 4198400 %33 "[nzsl_version("1.1")]
[author("SirLynix")]
[desc("Test color module")]
[license("MIT")]
module Color;

[set(0)]
external
{
    [binding(0)] tex1: sampler2D[f32]
}

fn GenerateColor() -> vec4[f32]
{
    return tex1.Sample(0.0.xx);
}

alias GenColor = GenerateColor;

[set(1)]
external
{
    [binding(0)] tex2: sampler2D[f32]
}

fn GenerateAnotherColor() -> vec4[f32]
{
    return tex2.Sample(0.5.xx);
}

[export]
fn GetColor() -> vec4[f32]
{
    return GenColor();
}

[export]
fn GetAnotherColor() -> vec4[f32]
{
    return GenerateAnotherColor();
}
"
      OpSourceExtension "Version: 1.1"
      OpSourceExtension "ModuleName: Color"
      OpSourceExtension "Author: SirLynix"
      OpSourceExtension "Description: Test color module"
      OpSourceExtension "License: MIT"
      OpSource SourceLanguage(NZSL) 4194304
      OpSourceExtension "Version: 1.0"
      OpSourceExtension "ModuleName: DataStruct"
      OpSource SourceLanguage(NZSL) 4198400 %34 "[nzsl_version("1.1")]
module OutputStruct;

import * from DataStruct;

option ColorMultiplier: vec4[f32] = vec4[f32](0.5, 0.5, 0.5, 1.0);

[export]
fn GetColorFromData(data: Data) -> vec4[f32]
{
    return data.color * ColorMultiplier;
}

[export]
struct Output
{
    [location(0)] color: vec4[f32]
}
"
      OpSourceExtension "Version: 1.1"
      OpSourceExtension "ModuleName: OutputStruct"
      OpName %11 "Data"
      OpMemberName %11 0 "color"
      OpName %19 "PushConstants"
      OpMemberName %19 0 "color"
      OpName %26 "Output"
      OpMemberName %26 0 "color"
      OpName %5 "tex1"
      OpName %21 "ExternalResources_constants"
      OpName %25 "color"
      OpName %28 "GenerateColor"
      OpName %29 "GetColor"
      OpName %30 "GetColorFromData"
      OpName %31 "main"
      OpDecorate %5 Decoration(Binding) 0
      OpDecorate %5 Decoration(DescriptorSet) 0
      OpDecorate %25 Decoration(Location) 0
      OpMemberDecorate %11 0 Decoration(Offset) 0
      OpDecorate %19 Decoration(Block)
      OpMemberDecorate %19 0 Decoration(Offset) 0
      OpMemberDecorate %26 0 Decoration(Offset) 0
 %1 = OpTypeFloat 32
 %2 = OpTypeImage %1 Dim(Dim2D) 0 0 0 1 ImageFormat(Unknown)
 %3 = OpTypeSampledImage %2
 %4 = OpTypePointer StorageClass(UniformConstant) %3
 %6 = OpTypeVector %1 4
 %7 = OpTypeFunction %6
 %8 = OpTypeVector %1 2
 %9 = OpConstant %1 f32(0)
%10 = OpConstantComposite %8 %9 %9
%11 = OpTypeStruct %6
%12 = OpTypePointer StorageClass(Function) %11
%13 = OpTypeFunction %6 %12
%14 = OpTypeInt 32 1
%15 = OpConstant %14 i32(0)
%16 = OpConstant %1 f32(0.5)
%17 = OpConstant %1 f32(1)
%18 = OpConstantComposite %6 %16 %16 %16 %17
%19 = OpTypeStruct %6
%20 = OpTypePointer StorageClass(PushConstant) %19
%22 = OpTypeVoid
%23 = OpTypeFunction %22
%24 = OpTypePointer StorageClass(Output) %6
%26 = OpTypeStruct %6
%27 = OpTypePointer StorageClass(Function) %26
%42 = OpTypePointer StorageClass(Function) %6
%54 = OpTypePointer StorageClass(PushConstant) %6
 %5 = OpVariable %4 StorageClass(UniformConstant)
%21 = OpVariable %20 StorageClass(PushConstant)
%25 = OpVariable %24 StorageClass(Output)
      OpLine %33 13 1
%28 = OpFunction %6 FunctionControl(0) %7
%35 = OpLabel
      OpNoLine
      OpLine %33 15 5
      OpLine %33 15 12
%36 = OpLoad %3 %5
%37 = OpImageSampleImplicitLod %6 %36 %10
      OpReturnValue %37
      OpFunctionEnd
      OpLine %33 32 1
%29 = OpFunction %6 FunctionControl(0) %7
%38 = OpLabel
      OpNoLine
      OpLine %33 34 5
      OpLine %33 34 12
%39 = OpFunctionCall %6 %28
      OpReturnValue %39
      OpFunctionEnd
      OpLine %34 9 1
%30 = OpFunction %6 FunctionControl(0) %13
      OpLine %34 9 21
%40 = OpFunctionParameter %12
%41 = OpLabel
      OpNoLine
      OpLine %34 11 5
      OpLine %34 11 12
%43 = OpAccessChain %42 %40 %15
%44 = OpLoad %6 %43
%45 = OpFMul %6 %44 %18
      OpReturnValue %45
      OpFunctionEnd
      OpLine %32 23 1
%31 = OpFunction %22 FunctionControl(0) %23
%46 = OpLabel
      OpNoLine
      OpLine %32 25 5
%47 = OpVariable %12 StorageClass(Function)
      OpLine %32 28 5
%48 = OpVariable %27 StorageClass(Function)
      OpLine %32 29 37
%49 = OpVariable %12 StorageClass(Function)
      OpLine %32 26 18
%50 = OpFunctionCall %6 %29
      OpLine %32 26 5
%51 = OpAccessChain %42 %47 %15
      OpStore %51 %50
      OpLine %32 29 37
%52 = OpLoad %11 %47
      OpStore %49 %52
      OpLine %32 29 20
%53 = OpFunctionCall %6 %30 %49
      OpLine %32 29 45
%55 = OpAccessChain %54 %21 %15
%56 = OpLoad %6 %55
      OpLine %32 29 20
%57 = OpFMul %6 %53 %56
      OpLine %32 29 5
%58 = OpAccessChain %42 %48 %15
      OpStore %58 %57
      OpLine %32 31 12
%59 = OpLoad %26 %48
%60 = OpCompositeExtract %6 %59 0
      OpStore %25 %60
      OpLine %32 31 5
      OpReturn
      OpFunctionEnd)", options, {}, true);
		}
	}
}
